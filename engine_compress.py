...
def train_one_epoch(model, criterion, train_dataloader, optimizer, aux_optimizer, epoch, loss_scaler, clip_max_norm, writer, args):
    ...
    for i, (samples, _, total_scores) in enumerate(metric_logger.log_every(train_dataloader, print_freq, header)):
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    model.eval()
    with torch.no_grad():
        for (samples, _, total_scores) in metric_logger.log_every(test_dataloader, 10, header):
            samples = samples.to(device)
            total_scores = total_scores.to(device)
