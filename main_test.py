...
def load_checkpoint(num_keep_patches: int, checkpoint_path: str) -> nn.Module:
    state_dict = load_state_dict(torch.load(checkpoint_path, map_location='cpu')['model'])
...
def eval_model(
    ...
    test_dataset = get_image_dataset(mode="test", args=args)
...
def setup_args():
    ...
    parser.add_argument("-t", "--total_scores_path", type=str, help="Path total_scores to the dataset")
